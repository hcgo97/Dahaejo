...
<%
String sqlTot = "select count(*) total from oneonone";
...
<table border="0" width="1000" align="center">
	<tr>
		<td align="right">
			<button type="button" style="cursor:pointer" onclick="location='oneOnOneWrite.jsp'">글쓰기</button>
		</td>
	</tr>
</table>
...
<table border="1" width="1000" align="center" style="border-collapse:collapse; border: 1px solid black;">
	...
	String sql = "  select b.* from  ( "
			   ...
			   + "		( select unq,title,name,hits,"
			   + " 			to_char(rdate,'yyyy-mm-dd') rdate, gid, insort from oneonone "
			   + "		        order by gid desc, insort asc) a ) b "
	...
	while( rs.next() ) {
		...
		int len = insort.length(); //답변갯수만큼 스페이스바추가
	%>
		<tr>
			...
			<td align="center" style="padding:10px 10px 10px 10px 10px;">
			 <% 
	         for(int i=1; i<len; i++) {
	            out.print("&emsp;&emsp;&emsp;&emsp;");
	         }
	         if( len > 1 ) {
	            out.print("ㄴ"); // [답글]
	         }
	         %>
			<a href="oneOnOneDetail.jsp?unq=<%=unq %>&name=<%=name %>"><%=title %></a>
			</td>
			<td align="center" style="padding:10px 10px 10px 10px 10px;"><%=name %></td>
			<td align="center" style="padding:10px 10px 10px 10px 10px;"><%=hits %></td>
			<td align="center" style="padding:10px 10px 10px 10px 10px;"><%=rdate %></td>
...
<table border="0" width="600" align="center">
	<tr>
		...
		%>
			  <a href="oneOnOneList.jsp?page=<%=i %>"><%=i %></a> 
		<%
		}
		%>
