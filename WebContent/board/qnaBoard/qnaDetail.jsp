...
<%
// 조회수 증가 -> hits
String sqlUpdate = " update qnaboard set hits=hits+1 "
				 + "  where unq='"+unq+"' " ;
stmt.executeUpdate(sqlUpdate);

String sql = " select title,to_char(rdate,'yyyy-mm-dd') as rdate,hits,content,gid,insort from qnaboard "
		   + "   where unq='"+unq+"'  ";
			...
			<nav id="nav">
				...
				<br> <br> <br>
					...
					<tr>
						...
						<button type="button" style="cursor:pointer" onclick=" location='qnaModify.jsp?unq=<%=unq %>&name=<%=name %>'">수정하기</button>
						<button type="button" style="cursor:pointer" onclick=" location='qnaDelete.jsp?unq=<%=unq %>&name=<%=name %>&gubun=2'">삭제하기</button>
				...
				String sql2 = " select replyer,replytext,to_char(redate,'yy-mm-dd hh24:mi') redate,replyunq from qna_reply "
				   ...
				   <table align = "center" border="0" width="600" >
				         <tr>
				                   <td>
				                    ...
				                    <td>
				                        <button type = "submit" id="reply_save" name="reply_save" align = "center" style="margin-right:600px;" onclick="fn_replysubmit();return false;">등록</button>
				                    </td>
